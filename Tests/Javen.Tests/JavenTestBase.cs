...
using Javen.Migrations.SeedData;
...
namespace Javen.Tests
{
    public abstract class JavenTestBase : AbpIntegratedTestBase<JavenTestModule>
    {
        ...
        protected void UsingDbContext(Action<JavenDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<JavenDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<JavenDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<JavenDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<JavenDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<JavenDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<JavenDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<JavenDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<JavenDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<JavenDbContext, Task<T>> func)
        {
            T result;
